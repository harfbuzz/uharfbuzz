cdef class HBObject:
    """Represents an array of objects in the object graph to be serialized.
    Used internally by :func:`serialize` and :func:`serialize_with_tag`.

    Wraps `hb_subset_serialize_object_t
    <https://harfbuzz.github.io/harfbuzz-hb-subset.html#hb-subset-serialize-object-t>`_.
    """
...
def serialize(subtables: List[bytes],
    ...
    """Same as :func:`serialize_with_tag` with an empty ``tag``, which
    disables table specific optimizations.
    """
...
def serialize_with_tag(tag: str,
    ...
    offset overflows and serialize it into a continuous array of bytes.
    Table specific optimizations (e.g. extension promotion in ``GSUB`` and
    ``GPOS``) may be performed. Passing an empty ``tag`` will disable
    ...
    :param tag: Tag of the table being packed, needed to allow table
        specific optimizations.
    :param subtables: List of subtable byte buffers, one per object in the
        object graph.
    :param graphnodes: List of ``(real_links, virtual_links)`` tuples, one
        per object.

    :returns: The serialized table as bytes.

    :raises RepackerError: If serialization fails.

    Wraps `hb_subset_serialize_or_fail()
    <https://harfbuzz.github.io/harfbuzz-hb-subset.html#hb-subset-serialize-or-fail>`_.
    """
